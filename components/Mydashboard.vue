<template>
  <v-layout>
    <v-flex>
      <h3 v-if="nodata" class="mt-5 font-weight-light">
      ...
      <div v-if="loaded">
      <h1 class="headline mb-5">{{partner_name}}</h1>
      <br />
        <v-row v-if="loaded">
  ...
  methods: {
    ...
    //   }
      ...
      try {
        ...
        const dash = await this.$axios.$get('/admin/dashboard/get/' + this.$store.state.sessionStorage.code)
        if (dash) {
          this.datapresent = false
        }
        this.partner_name = dash.partner_name
